...
folly::Expected<std::shared_ptr<IUbiDevice>, int32_t> UbiDevice::Create(
  ...
  if (create_mtd_table_result.hasError()) {
    XLOG(ERR) << "MtdTable::Create failed! error code = "
  ...
  if (get_mtd_num_result.hasError()) {
    XLOG(ERR) << "GetMtdNum failed! error code = "
...
UbiDevice::CheckKernelSupportForAttachDetachRequest(libubi_t lib_ubi_fd) {
  ...
  if (ret) {
    XLOG(ERR) << "ubi_get_info failed! error code = " << ret;
    return folly::makeUnexpected(
        ErrorCode::CHECK_KERNEL_SUPPORT__CANNOT_GET_UBI_INFORMATION_ERROR);
  }

  if (ubi_info.ctrl_major == -1) {
    XLOG(ERR) << "Mtd Attach Detach feature is not supported by the kernel";
    return folly::makeUnexpected(
        ErrorCode::
...
// Attach
folly::Expected<folly::Unit, int32_t> UbiDevice::Attach(void) {
  int ret;
  ...
  if (get_ubi_lib_fd_result.hasError()) {
    XLOG(ERR) << "CreateUbiLibFileHandle failed! error code = "
  ...
  if (kernel_support_result.hasError()) {
    ...
    return folly::makeUnexpected(int(kernel_support_result.error()));
  }
  ...
  if (ret < 0) {
    XLOG(ERR) << "ubi_attach failed! error code = " << ret
              << " mtd_num=" << mtd_num_;
    return folly::makeUnexpected(
        int(ErrorCode::ATTACH__CANNOT_ATTACH_MTD_DEVICE));
  }
  ...
  if (ret) {
    XLOG(ERR) << "mtd_num2ubi_dev failed! error code = " << ret
              << " mtd_num=" << mtd_num_;
    return folly::makeUnexpected(int(ErrorCode::ATTACH__MTD_NUM_TO_UBI_ERROR));
  }
...
// Detach
folly::Expected<folly::Unit, int32_t> UbiDevice::Detach(void) {
  int ret = 0;
  ...
  if (ret < 0) {
    XLOG(ERR) << "ubi_detach_mtd failed! error code = " << ret
              << " mtd_num_=" << mtd_num_;
    return folly::makeUnexpected(
        int(ErrorCode::DETACH__CANNOT_DETACH_MTD_DEVICE_ERROR));
  }
...
folly::Expected<folly::Unit, int32_t> UbiDevice::MakeVolume(
  ...
  if (ubi_probe_node_result.hasError()) {
    XLOG(ERR) << "UbiProbeNode failed! error code = "
  ...
  if (ret) {
    XLOG(ERR) << "ubi_get_dev_info failed! cannot get information about UBI "
  ...
  if (dev_info.avail_bytes == 0) {
    XLOG(ERR) << "UBI device does not have free logical eraseblocks. "
                 "ubi_device_file_name_="
              << ubi_device_file_name_;
  ...
  if (ret < 0) {
    XLOG(ERR) << "ubi_mkvol failed! ubi_device_file_name_="
...
folly::Expected<folly::Unit, int32_t> UbiDevice::RemoveVolume(
  ...
  if (ret) {
    if (is_to_print_log_error) {
      XLOG(ERR) << "ubi_rmvol_nm failed! cannot UBI remove volume.  UBI device="
...
CreateUbiLibFileHandle() {
  ...
  if (!libubi_fd) {
    if (errno == 0) {
      XLOG(ERR) << "libubi_open failed! UBI is not present in the system";
      return folly::makeUnexpected(
          UbiDevice::ErrorCode::OPEN_LIB_UBI__UBI_IS_NOT_PRESENT_IN_THE_SYSTEM);
    }
    XLOG(ERR) << "libubi_open failed! cannot open libubi";
    return folly::makeUnexpected(
...
CreateMtdLibFileHandle() {
  ...
  if (!libmtd_fd) {
    if (errno == 0) {
      XLOG(ERR) << "libmtd_open failed! MTD is not present in the system";
      return folly::makeUnexpected(
          UbiDevice::ErrorCode::OPEN_LIB_MTD__MTD_IS_NOT_PRESENT_IN_THE_SYSTEM);
    }
    XLOG(ERR) << "libmtd_open failed! cannot open libmtd";
    return folly::makeUnexpected(
...
CreateCStyleFileHandle(std::string file_name, int mode) {
  ...
  if (fd == -1) {
    XLOG(ERR) << "open " << file_name << " failed! mode= " << mode;
    return folly::makeUnexpected(
...
folly::Expected<folly::Unit, UbiDevice::ErrorCode> UbiDevice::Format(
  ...
  if (get_mtd_lib_fd_result.hasError()) {
    XLOG(ERR) << "CreateMtdLibFileHandle failed! error code = "
  ...
  if (ret) {
    XLOG(ERR) << "mtd_get_dev_info failed! cannot get information about "
  ...
  if (!is_power_of_2(mtd.min_io_size)) {
    XLOG(ERR) << "min. I/O size is " << mtd.min_io_size
  ...
  if (!mtd_info.sysfs_supported) {
     ...
     */
    XLOG(WARN) << "your MTD system is old and it is impossible to detect "
  ...
  if (create_c_style_fd_result.hasError()) {
    XLOG(ERR) << "CreateCStyleFileHandle failed! error code = "
  ...
  if (!mtd.writable) {
    XLOG(ERR) << mtd_device_file_name << " is a read-only device";
    return folly::makeUnexpected(
  ...
  if (!ret) {
    XLOG(ERR) << "mtd" << mtd.mtd_num << "is already attached to ubi"
  ...
  if (ret) {
    XLOG(ERR) << "ubi_scan failed! failed to scan mtd" << mtd.mtd_num;
    return folly::makeUnexpected(ErrorCode::FORMAT__UBI_SCAN_FAILURE_ERROR);
  }

  // erase blocks check
  if (si->good_cnt == 0) {
    XLOG(ERR) << "ubi_scan mtd" << mtd.mtd_num << " all " << si->bad_cnt
              << " eraseblocks are bad";
    return folly::makeUnexpected(
  ...
  if (si->good_cnt < 2) {
    XLOG(ERR) << "ubi_scan mtd" << mtd.mtd_num
  ...
  if (si->alien_cnt) {
    XLOG(WARN) << si->alien_cnt << " of " << si->good_cnt
  ...
  if (si->empty_cnt < si->good_cnt) {
     ...
     */
    if (percent < 50) {
      XLOG(WARN) << "only " << si->ok_cnt << " of " << si->good_cnt
  ...
  if (si->vid_hdr_offs != -1 && ui.vid_hdr_offs != si->vid_hdr_offs) {
     ...
     */
    XLOG(WARN) << "VID header and data offsets on flash are "
               << si->vid_hdr_offs << " and " << si->data_offs
               << " which is different to requested offsets " << ui.vid_hdr_offs
               << " and " << ui.data_offs << ". use new offsets "
  ...
  if (do_format_result.hasError()) {
    XLOG(ERR) << "FormatExec failed! error code = "
...
folly::Expected<folly::Unit, UbiDevice::ErrorCode> UbiDevice::FormatExec(
  ...
  for (int eb = start_eb; eb < mtd->eb_cnt; eb++) {
    ...
    if (ret) {
      XLOG(ERR) << "failed to erase eraseblock=" << eb << "ret=" << ret
                << "errno=" << errno;
      if (errno != EIO) {
        return folly::makeUnexpected(
      ...
      if (mark_bad_result.hasError()) {
        XLOG(ERR) << "MarkBadBlocks failed! error code = "
    ...
    if (ret) {
      XLOG(ERR) << "cannot write EC header (" << write_size
                << " bytes buffer) to eraseblock " << eb << "ret=" << ret
                << "errno=" << errno;

      if (errno != EIO) {
        if (format_attr.subpage_size != mtd->min_io_size) {
          XLOG(ERR) << "may be sub-page size is incorrect?";
          return folly::makeUnexpected(
  ...
  if (eb1 == -1 || eb2 == -1) {
    XLOG(ERR) << "no eraseblocks for volume table";
    return folly::makeUnexpected(
  ...
  if (!vtbl) {
    XLOG(ERR) << "ubigen_create_empty_vtbl failed!";
    return folly::makeUnexpected(
  ...
  if (ret) {
    XLOG(ERR) << "cannot write layout volume";
...
folly::Expected<folly::Unit, UbiDevice::ErrorCode> UbiDevice::MarkBadBlocks(
  ...
  if (!mtd->bb_allowed) {
    XLOG(ERR) << "bad blocks not supported by this flash";
    return folly::makeUnexpected(
  ...
  if (ret) {
    XLOG(ERR) << "mtd_mark_bad failed! ret=" << ret << "eb=" << eb;
  ...
  if (consecutive_bad_check_result.hasError()) {
    XLOG(ERR) << "ConsecutiveBadBlocksCheck failed! error code = "
              << int(consecutive_bad_check_result.error()) << " eb=" << eb;
    return folly::makeUnexpected(
...
UbiDevice::ConsecutiveBadBlocksCheck(int eb) {
  ...
  if (consecutive_bad_blocks >= kMaxConsecutiveBadBlocks) {
    XLOG(ERR) << "consecutive bad blocks exceed limit: "
...
folly::Expected<folly::Unit, int32_t> UbiDevice::UpdateVolume(
  ...
  if (!f.good()) {
    XLOG(ERR) << ubifs_image_file_str << " not exist!";
    return folly::makeUnexpected(
  ...
  if (size > 0) {
    ...
    if (ret < 0) {
      XLOG(ERR) << "stat failed on " << ubifs_image_file_str;
      return folly::makeUnexpected(
  ...
  if (bytes > vol_info.rsvd_bytes) {
    XLOG(ERR) << ubifs_image_file_str << " size=" << bytes
  ...
  if (skip_bytes > 0) {
    if (lseek(fd_image, skip_bytes, SEEK_CUR) == -1) {
      XLOG(ERR) << "lseek input by " << skip_bytes << " failed!";
      return folly::makeUnexpected(
  ...
  if (ret) {
    XLOG(ERR) << "ubi_update_start failed! cannot start volume "
  ...
  while (bytes) {
    ...
    if (size <= 0) {
      if (errno == EINTR) {
        XLOG(ERR) << "do not interrupt me!";
        continue;
      } else {
        XLOG(ERR) << "cannot read " << to_copy << " bytes from "
    ...
    if (ubi_write_result.hasError()) {
      XLOG(ERR) << "UbiWrite failed! error code = "
  ...
  XLOG(INFO) << "UBI update volume operation finished successfully"
...
folly::Expected<folly::Unit, UbiDevice::ErrorCode> UbiDevice::UbiWrite(
  ...
  while (size) {
    ...
    if (ret_size < 0) {
      if (errno == EINTR) {
        XLOG(WARN) << "do not interrupt me!";
        continue;
      }

      XLOG(ERR) << "cannot write " << size << " bytes to volume "
                << ubi_volume_file_name;
...
folly::Expected<folly::Unit, UbiDevice::ErrorCode> UbiDevice::UbiProbeNode(
  ...
  if (ret == 2) {
    if (is_to_print_log_error) {
      XLOG(ERR) << "ubi_probe_node failed! " << ubi_device_file_name_
    ...
    if (errno == ENODEV) {
      ...
      if (is_to_print_log_error) {
        XLOG(ERR) << "ubi_probe_node failed! "
                  << " ubi_device_file_name_=" << ubi_device_file_name_
                  << " ret=" << ret;
      }
...
folly::Expected<folly::Unit, int32_t> UbiDevice::MountVolume(
  ...
  if (get_ubi_vol_file_result.hasError()) {
    XLOG(ERR) << "GetUbiVolumeFile failed! error code = "
  ...
  // verify a dir for mount exists
  if (!std::filesystem::exists(dir_to_mount)) {
    std::error_code error_code;
    if (!std::filesystem::create_directory(dir_to_mount, error_code)) {
      XLOG(ERR) << "std::filesystem::create_directory failed! dir="
                << dir_to_mount << " error_code=" << error_code;
      return folly::makeUnexpected(
          int(ErrorCode::MOUNT_VOLUME__CREATE_DIR_FAILED_ERROR));
    }
  }
  ...
  int ret = mount(ubi_volume_file_name.c_str(), dir_to_mount.c_str(), "ubifs",
                  0, NULL);
  if (ret) {
    XLOG(ERR) << "mount failed! "
              << " ubi_volume_full_file_name_=" << ubi_volume_file_name
              << " dir_to_mount=" << dir_to_mount << " errno=" << errno;
    return folly::makeUnexpected(
...
folly::Expected<folly::Unit, int32_t> UbiDevice::UnmountVolume(
  ...
  if (ret) {
    XLOG(ERR) << "umount failed! "
              << "dir_to_unmount=" << dir_to_unmount << "errno=" << errno;
    return folly::makeUnexpected(
...
folly::Expected<folly::Unit, int32_t> UbiDevice::Format(void) {
  auto format_result = Format(mtd_num_);
  if (format_result.hasError()) {
    XLOG(ERR) << "Format failed! "
              << "mtd_num_=" << mtd_num_
              << "error=" << int(format_result.error());

    return folly::makeUnexpected(int(format_result.error()));
  }
